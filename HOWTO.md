...
> ⚠️ **Important:** AWS Free Tier does not mean "free no matter what." Some services are free **only for the first 12 months**, others are **always free up to a quota**, and some are **never free** (NAT Gateways, anything in a "Pro" tier). The HOWTO sticks to free services, but **you must complete Phase 8 (cleanup)** when the lab is done.
...
The umbrella term for what we're building is CSPM (Cloud Security Posture Management), and the design philosophy we're following is defense in depth — we don't trust any single layer to catch everything; we layer logging + detection + posture + automation so a failure in one layer is caught by the next.
...
- Search "IAM" → **Users** → Create user → name `your-name-admin` (or your preferred name).
...
IAM → Users → your admin user → Security credentials → Assign MFA → Authenticator app.
...
1. S3 → Create bucket
2. Name: `yourname-cloudsec-logs-<random6digits>` (S3 names are globally unique)
3. Region: `us-east-1`
...
> 📝 *Region note:* CloudTrail sign-in events are generated in `us-east-1` (IAM is global). Make sure this EventBridge rule is also created in `us-east-1` so it can see those events.
...
Lambda functions don't have permissions by default — they assume an IAM role. We follow least privilege: the role only gets to do exactly what it needs.
...
```json
{
  ...
  "Statement": [{
    ...
    "Principal": "*",
    "Action": "s3:GetObject",
    "Resource": "arn:aws:s3:::cloudsec-test-public-<your-bucket-suffix>/*"
  }]
}
```
...
4. Try to create an IAM user without MFA → verify Config flags it (note: Config evaluates this periodically — use Actions → Re-evaluate to trigger it immediately).
...
Save these PNGs into the `screenshots/` folder:

- [ ] AWS Budgets dashboard showing $0 budget
- [ ] CloudTrail Event history showing your `ConsoleLogin` event
- [ ] GuardDuty Findings page with your sample findings
- [ ] AWS Config compliance dashboard
- [ ] EventBridge rule with the JSON pattern visible
- [ ] An alert email in your inbox (redact your account ID and email address!)
- [ ] CloudWatch Logs showing the Lambda's remediation run
- [ ] The S3 bucket flipping back to "all public access blocked"

### 7.2 — Architecture diagram

The Mermaid diagram in README.md renders automatically on GitHub. For a more polished version with AWS icons, import the diagram into [draw.io](https://app.diagrams.net) or [Excalidraw](https://excalidraw.com), export as PNG, and embed in README.md.
...
git branch -M main
git remote add origin https://github.com/<your-github-username>/aws-cloud-security-lab.git
...
2. Configure AWS CLI with an access key for a *new* IAM user with limited permissions (not your admin user)
...
| Root login alert not firing | The EventBridge rule must be in `us-east-1` — that's where IAM sign-in events are generated. |
...
- **Concept depth:** defense in depth, CSPM, auto-remediation
